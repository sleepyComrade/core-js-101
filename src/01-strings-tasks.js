 ...
 */
const extractNameFromTemplate = (value) => value.split(',')[1].slice(0, -1).trim();
...
const getRectangleString = (width, height) => {
  const a = height > 2 ? `│${' '.repeat(width - 2)}│\n` : '';
 ...
 *
 */
const encodeToRot13 = (str) => str.split('').map((el) => {
  let a;
  if (el >= 'A' && el <= 'Z') {
    a = 'A'.charCodeAt() + ((el.charCodeAt() - 'A'.charCodeAt() + 13) % 26);
  } else if (el >= 'a' && el <= 'z') {
    a = 'a'.charCodeAt() + ((el.charCodeAt() - 'a'.charCodeAt() + 13) % 26);
  } else a = el.charCodeAt();
  return a;
